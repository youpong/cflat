...
public class CommandUtils {
    static public void invoke(List<String> cmdArgs, ErrorHandler errorHandler,
            boolean verbose) throws IPCException {
        if (verbose) {
            dumpCommand(cmdArgs);
        }
        try {
            ...
            if (proc.exitValue() != 0) {
                errorHandler.error(
                        cmd[0] + " failed." + " (status " + proc.exitValue() + ")");
                throw new IPCException("compile error");
            }
        } catch (InterruptedException ex) {
            errorHandler.error("external command interrupted: " + cmdArgs.get(0)
                    + ": " + ex.getMessage());
            throw new IPCException("compile error");
